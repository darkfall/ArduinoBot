...
{	
	/*
	 * Commented for now, in order to check if the hang bug comes from here ...
	 *
	 */
/*
 ...
 */
	// Need to keep this, even with the storing part commmented
	...
	/*
	bt_accessor(0)->buffers.inbuff = xrealloc(bt_accessor(0)->buffers.inbuff, (bt_accessor(0)->buffers.inbuff_len + length + 1) * sizeof(*packet));
	memcpy((bt_accessor(0)->buffers.inbuff + bt_accessor(0)->buffers.inbuff_len),  &packet[start_of_data], length * sizeof(*packet));
...
int		hci_inquiry_complete(uint8_t *packet, uint16_t size)
{
  ...
  if (bt_accessor(0)->scanretries < 1)
